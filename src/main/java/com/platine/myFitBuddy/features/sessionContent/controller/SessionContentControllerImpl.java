...
public class SessionContentControllerImpl implements SessionContentController {
  ...
  public ResponseEntity<List<SessionContent>> findByUserBySessionId(Long sessionId) {
    return ResponseEntity.ok(
      sessionContentService.findByUserIdBySession(
        sessionId,
  ...
  @Override
  public ResponseEntity<SessionContent> update(
    SessionContentUpdateForm updateForm,
    Long sessionId
  ) {
    return ResponseEntity.ok(
      sessionContentService.update(updateForm, sessionId, dbUserService.getCurrentUser())
    );
  }
}
