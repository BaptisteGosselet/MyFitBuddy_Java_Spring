...
public class FitRecordServiceImplTest {
  ...
  void getRecordByIdTest() {
    ...
    when(fitRecordRepository.findById(fitRecordId)).thenReturn(Optional.of(fitRecordExpected));

    Optional<FitRecord> fitRecordFromService = fitRecordServiceImpl.getRecordById(fitRecordId,user);
    ...
    assertEquals(fitRecordExpected.getUser().getUsername(), fitRecordFromService.get().getUser().getUsername());
  }
  ...
  void getRecordByUserTest() {
    ...
    for(int i=0;i<fitRecordsExpected.size();i++){
      assertEquals(fitRecordsExpected.get(i).getName(),fitRecordFromService.get(i).getName());
      assertEquals(fitRecordsExpected.get(i).getId(),fitRecordFromService.get(i).getId());
      assertEquals(fitRecordsExpected.get(i).getUser().getUsername(),fitRecordFromService.get(i).getUser().getUsername());
    }
  }
  ...
  void createRecordTest() {
    ...
    assertEquals(fitRecordExpected.getUser().getUsername(), fitRecordfromService.getUser().getUsername());
  }
  ...
  void deleteRecordTest() {
    ...
    when(fitRecordRepository.findById(anyLong())).thenReturn(Optional.of(fitRecordExpected));
