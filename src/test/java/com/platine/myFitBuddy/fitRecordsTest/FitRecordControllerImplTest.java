...
public class FitRecordControllerImplTest {
  ...
  void getRecordByIdTest() throws Exception {
    ...
    when(fitRecordService.getRecordById(anyLong(), any()))
      .thenReturn(Optional.of(fitRecordExpected));

    MvcResult result =
      this.mockMvc.perform(get("/records/" + fitRecordId))
  ...
  void getRecordByUserTest() throws Exception {
    ...
    MvcResult result =
      this.mockMvc.perform(get("/records/user")).andExpect(status().isOk()).andReturn();

    String json = result.getResponse().getContentAsString();
    List<FitRecord> fitRecordFromController = objectMapper.readValue(
      json,
      new TypeReference<>() {}
    );
    ...
    for (int i = 0; i < fitRecordsExpected.size(); i++) {
      assertEquals(
        ...
        fitRecordFromController.get(i).getName()
      );
      assertEquals(
        fitRecordsExpected.get(i).getId(),
        fitRecordFromController.get(i).getId()
      );
      assertEquals(
        fitRecordsExpected.get(i).getUser().getUsername(),
        fitRecordFromController.get(i).getUser().getUsername()
      );
    }
  }
  ...
  void createRecordTest() throws Exception {
    ...
    MvcResult result =
      this.mockMvc.perform(post("/records/create/" + sessionId))
        .andExpect(status().isOk())
        .andReturn();

    String json = result.getResponse().getContentAsString();
    FitRecord fitRecordFromController = objectMapper.readValue(
      json,
      new TypeReference<>() {}
    );
    ...
    assertEquals(
      ...
      fitRecordFromController.getUser().getUsername()
    );
  }
  ...
  void deleteRecordTest() throws Exception {
    ...
    this.mockMvc.perform(delete("/records/delete/" + sessionId))
      .andExpect(status().isOk());
