...
public class ExerciseControllerImplTest {
  ...
  void getExercisesTest() throws Exception {
    ...
    List<Exercise> exerciceToExpect = List.of(
      new Exercise(key, muscleGroup, "en", "fr"),
      new Exercise(key, muscleGroup, "EN", "FR")
    );
    Pageable pageable = PageRequest.of(0, 20);
    Page<Exercise> pageExercice = new PageImpl<>(
      exerciceToExpect,
      pageable,
      exerciceToExpect.size()
    );

    when(exerciseService.findByKeyAndMuscleGroup(key, muscleGroup, pageable))
      .thenReturn(pageExercice);

    MvcResult result =
      this.mockMvc.perform(
          get("/exercises")
            .param("key", key)
            .param("muscleGroup", String.valueOf(muscleGroup))
        )
  ...
  void getExerciseByIdTest() throws Exception {
    ...
    MvcResult result =
      this.mockMvc.perform(get("/exercises/" + exerciceId))
        .andExpect(status().isOk())
        .andReturn();
    ...
    assertEquals(
      exerciceToExpect.getMuscleGroup(),
      exerciceFromController.getMuscleGroup()
    );
  ...
  void getImageTest() throws Exception {
    ...
    this.mockMvc.perform(get("/exercises/" + exerciseId + "/image"))
      .andExpect(status().isOk())
