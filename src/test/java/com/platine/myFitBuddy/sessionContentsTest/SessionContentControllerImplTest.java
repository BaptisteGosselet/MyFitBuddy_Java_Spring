...
public class SessionContentControllerImplTest {
  ...
  void findByUserBySessionIdTest() throws Exception {
    ...
    List<SessionContentDTOWithExercise> listSessionContentExpected = List.of(
      new SessionContentDTOWithExercise(
        1L,
        1L,
    ...
    when(sessionContentService.findByUserIdBySession(anyLong(), any()))
      .thenReturn(listSessionContentFromService);
    when(exerciseService.getExerciseById(anyLong())).thenReturn(exercise);

    MvcResult result =
      this.mockMvc.perform(get("/sessionContent/" + sessionId))
        .andExpect(status().isOk())
        .andReturn();

    String json = result.getResponse().getContentAsString();
    List<SessionContentDTOWithExercise> sessionContentFromController = objectMapper.readValue(
      json,
      new TypeReference<>() {}
    );
    ...
    for (int i = 0; i < sessionContentFromController.size(); i++) {
      assertEquals(
        listSessionContentExpected.get(i).getSessionId(),
        sessionContentFromController.get(i).getSessionId()
      );
      assertEquals(
        listSessionContentExpected.get(i).getExercise().getKey(),
        sessionContentFromController.get(i).getExercise().getKey()
      );
      assertEquals(
        listSessionContentExpected.get(i).getNumberOfSet(),
        sessionContentFromController.get(i).getNumberOfSet()
      );
      assertEquals(
        listSessionContentExpected.get(i).getRestTimeInSecond(),
        sessionContentFromController.get(i).getRestTimeInSecond()
      );
      assertEquals(
        listSessionContentExpected.get(i).getIndex(),
        sessionContentFromController.get(i).getIndex()
      );
    }
  }
  ...
  void createTest() throws Exception {
    ...
    SessionContentCreateForm sessionContentCreateForm = new SessionContentCreateForm(
      sessionContent.getSessionId(),
      sessionContent.getExerciseId(),
      sessionContent.getNumberOfSet(),
      sessionContent.getRestTimeInSecond()
    );
    ...
    MvcResult result =
      this.mockMvc.perform(
          post("/sessionContent")
            ...
            .content(objectMapper.writeValueAsString(sessionContentCreateForm))
        )
        .andExpect(status().isOk())
        .andReturn();

    String json = result.getResponse().getContentAsString();
    SessionContentDTO sessionContentFromController = objectMapper.readValue(
      json,
      new TypeReference<>() {}
    );
    ...
    assertEquals(
      sessionContentExpected.getSessionId(),
      sessionContentFromController.getSessionId()
    );
    assertEquals(
      sessionContentExpected.getNumberOfSet(),
      sessionContentFromController.getNumberOfSet()
    );
    assertEquals(
      sessionContentExpected.getRestTimeInSecond(),
      sessionContentFromController.getRestTimeInSecond()
    );
  }
  ...
  void updateTest() throws Exception {
    ...
    when(sessionContentService.update(any(), anyLong(), any()))
      .thenReturn(sessionContent);
    System.out.println(sessionContentUpdateForm.toString());
    MvcResult result =
      this.mockMvc.perform(
          put("/sessionContent/" + sessionContentUpdateForm.getId())
            .contentType(MediaType.APPLICATION_JSON)
            .content(objectMapper.writeValueAsString(sessionContentUpdateForm))
        )
        .andExpect(status().isOk())
        .andReturn();
  ...
  void deleteTest() throws Exception {
    ...
    this.mockMvc.perform(delete("/sessionContent/" + sessionContentId))
      .andExpect(status().isOk());
  ...
  void changeOrderTest() throws Exception {
    ...
    DBUser user = new DBUser("username", "email", "password", "role");
    Exercise exercise = new Exercise("key", MuscleGroup.ARMS, "en", "fr");
    List<SessionContentUpdateForm> listSessionContentUpdateForm = List.of(
      new SessionContentUpdateForm(1L, 1, 1, 1),
      new SessionContentUpdateForm(1L, 1, 1, 1)
    );
    ...
    when(sessionContentService.update(any(), anyLong(), any()))
      .thenReturn(sessionContentFromService);

    MvcResult result =
      this.mockMvc.perform(
          put("/sessionContent/list")
            .contentType(MediaType.APPLICATION_JSON)
            .content(objectMapper.writeValueAsString(listSessionContentUpdateForm))
        )
        .andExpect(status().isOk())
        .andReturn();
