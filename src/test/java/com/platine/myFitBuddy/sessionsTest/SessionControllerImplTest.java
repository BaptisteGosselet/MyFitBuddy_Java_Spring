...
public class SessionControllerImplTest {
  ...
  void findByIdTest() throws Exception {
    ...
    MvcResult result =
      this.mockMvc.perform(get("/sessions/" + sessionId))
    ...
    assertEquals(
      fitSession.get().getUser().getUsername(),
      fitSessionFromController.getUser().getUsername()
    );
  }
  ...
  void findByUserTest() throws Exception {
    ...
    DBUser user = new DBUser("username", "email", "password", "role");
    List<FitSession> fitSession = List.of(
      new FitSession(name, user),
      new FitSession("name2", user)
    );

    when(sessionService.findByUserId(any())).thenReturn(fitSession);

    MvcResult result =
      this.mockMvc.perform(get("/sessions/user")).andExpect(status().isOk()).andReturn();

    String json = result.getResponse().getContentAsString();
    List<FitSession> fitSessionFromController = objectMapper.readValue(
      json,
      new TypeReference<>() {}
    );
    ...
    for (int i = 0; i < fitSession.size(); i++) {
      ...
      assertEquals(
        fitSession.get(i).getName(),
        fitSessionFromController.get(i).getName()
      );
      assertEquals(
        fitSession.get(i).getUser().getUsername(),
        fitSessionFromController.get(i).getUser().getUsername()
      );
    }
  }
  ...
  void findAllSessionsTest() throws Exception {
    ...
    MvcResult result =
      this.mockMvc.perform(get("/sessions/all")).andExpect(status().isOk()).andReturn();
  ...
  void createTest() throws Exception {
    ...
    MvcResult result =
      this.mockMvc.perform(
          post("/sessions")
            ...
            .content(objectMapper.writeValueAsString(sessionCreateForm))
        )
        .andExpect(status().isOk())
        .andReturn();
    ...
    assertEquals(
      fitSession.getUser().getUsername(),
      fitSessionFromController.getUser().getUsername()
    );
  }
    ...
    MvcResult result =
      this.mockMvc.perform(
          put("/sessions/" + sessionId)
            .contentType(MediaType.APPLICATION_JSON)
            .content(objectMapper.writeValueAsString(sessionUpdateForm))
        )
        .andExpect(status().isOk())
        .andReturn();
